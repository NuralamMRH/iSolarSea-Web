...
const Chanhvantai = () => {
  ...
  const fetchHighestBids = async (productOrders: ProductOrderWithImages[]) => {
    try {
      ...
      for (const order of productOrders) {
        if (order.catch_id) {
          try {
            ...
            console.error(`Error fetching bid for order ${order.id}:`, error);
            bids[order.id] = null;
          }
        }
      }
      ...
      console.error("Error in fetchHighestBids:", error);
      // Set empty bids to prevent blocking
      setHighestBids({});
    }
  };
      ...
      try {
        ...
        for (const code of REGION_CODES) {
          ...
          for (const vesselId in vesselMap) {
            ...
            // Get product orders with type "2ShareLoading" and sufficient stock
        ...
        // Debug logging
        console.log("Final regionVessels structure:", regionVessels);
        console.log("Total vessels across all regions:", Object.values(regionVessels).flat().length);
        console.log("Vessels with product orders:", Object.values(regionVessels).flat().filter(v => v.productOrders.length > 0).length);
        ...
        console.log("Total product orders for bid fetching:", allProductOrders.length);
        await fetchHighestBids(allProductOrders);
        console.log("Finished fetching highest bids");
      } catch (e: unknown) {
  ...
  const filteredRegionProducts = useMemo(() => {
    console.log("filteredRegionProducts useMemo triggered");
    console.log("regionProducts:", regionProducts);
    ...
    console.log("Vessels before grouping:", vessels.length);
    console.log("Vessels with product orders:", vessels.filter(v => v.productOrders.length > 0).length);

    // Group vessels by their region
    vessels.forEach((vessel) => {
      // Determine region based on vessel's trips departure port
      let vesselRegion = "A"; // Default region
      
      if (vessel.vesselTrips.length > 0) {
        const trip = vessel.vesselTrips[0];
        
        // Use to_region if available
        if (trip.to_region) {
          const regionChar = trip.to_region.charAt(0).toUpperCase();
          if (REGION_CODES.includes(regionChar)) {
            vesselRegion = regionChar;
          }
        }
      }

      // Add vessel to the determined region
      regionVessels[vesselRegion].push(vessel);
      
      console.log(`Vessel ${vessel.vessel.name} assigned to region ${vesselRegion}, has ${vessel.productOrders.length} orders`);
    });

    console.log("Final filtered regionVessels:", regionVessels);
    console.log("Total vessels in filtered result:", Object.values(regionVessels).flat().length);
      ...
      <div id="smooth-wrapper">
        <div id="smooth-content">
          <main id="main" className="main">
            <div ref={heroRef}>
              <PageHeroHeader
                heading={t("chanhvantaiPage.heading")}
                breadcrumbHome={t("chanhvantaiPage.breadcrumb_home")}
                breadcrumbCurrent={t("chanhvantaiPage.breadcrumb_current")}
              />
            </div>
            ...
            <div className="market">
                ...
                <div className="container">
                  <h2 className="c-title-1">
                    <span className="c-title-1__stroke">
                      {t("chanhvantaiPage.supply_zone")}
                    </span>
                    <br className="show_sp" />
                    <span className="c-title-1__gradient">
                      {t("chanhvantaiPage.supply_zone_sub")}
                    </span>
                  </h2>
                  ...
                  <div className="market__content bg-white">
                    <div className="market__left">
                      <img
                        src="/images/top/chanh2.png"
                        alt={t("chanhvantaiPage.img_alt1")}
                      />
                    </div>
                    <div className="market__left show_pc">
                      <img
                        src="/images/top/chanhvantai.png"
                        alt={t("chanhvantaiPage.img_alt2")}
                      />
                    </div>
                    <div className="market__map">
                      <img
                        src="/images/top/thumua2.png"
                        alt={t("chanhvantaiPage.map_alt")}
                      />
                    </div>
                  </div>
                  <div className="market__left show_sp text-center mt30">
                    <img
                      src="/images/top/chanhvantai.png"
                      alt={t("chanhvantaiPage.img_alt2")}
                    />
              ...
              <div className="market__box2" ref={box2Ref}>
                <div className="container">
                  <h2 className="c-title-1">
                    <span className="c-title-1__stroke">
                      {t("chanhvantaiPage.chanh")}
                    </span>
                    <span className="c-title-1__gradient">
                      {t("chanhvantaiPage.nhan_tai")}
                      <br />
                      iBoatShare
                    </span>
                  </h2>
                  ...
                  {regionLoading ? (
                    ...
                    (() => {
                      console.log("Showing demo cards because:");
                      console.log("filteredRegionProducts values:", Object.values(filteredRegionProducts));
                      console.log("Every region empty?", Object.values(filteredRegionProducts).every(
                        (vessels) => !vessels || vessels.length === 0
                      ));
                      Object.entries(filteredRegionProducts).forEach(([region, vessels]) => {
                        console.log(`Region ${region}: ${vessels ? vessels.length : 0} vessels`);
                        if (vessels && vessels.length > 0) {
                          vessels.forEach(v => {
                            console.log(`  - Vessel ${v.vessel.name}: ${v.productOrders.length} orders`);
                          });
                        }
                      });
                      return null;
                                ...
                                .map((vessel, vesselIdx) => (
                                  ...
                                  >
                                    <div className="market__box2-price">
                                      ...
                                      >
                                        {t("chanhvantaiPage.ship_info")}
                                      </motion.h4>
                                      <div className="market__box2-spilit">
                                        <div className="market__box2-fixed">
                                          <ul>
                                            <li>
                                              {t("chanhvantaiPage.vessel_id")}
                                            </li>
                                            <li>
                                              {t("chanhvantaiPage.trip_id")}
                                            </li>
                                            <li>
                                              {t("chanhvantaiPage.from_zone")}
                                            </li>
                                            <li>
                                              {t("chanhvantaiPage.to_port")}
                                            </li>
                                            <li>
                                              {t(
                                                "chanhvantaiPage.available_weight"
                                              )}
                                            </li>
                                            <li>
                                              {t("chanhvantaiPage.price")}
                                            </li>
                                            <li>
                                              {t("chanhvantaiPage.departure")}
                                            </li>
                                            <li>
                                              {t("chanhvantaiPage.arrival")}
                                            </li>
                                            <li></li>
                                          </ul>
                                        </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-blue-50 p-4 rounded-lg">
                    ...
                    <div className="mb-3">
                      ...
                      <p className="text-sm bg-white p-2 rounded border">
                        {getPortNameById(auctionDialog.productOrder.departure_port)} → {getPortNameById(auctionDialog.productOrder.to_port)}
                      </p>
                    </div>
...
function AuctionForm({
  ...
  async function fetchVessels() {
    try {
      ...
      // First, get all product orders with type "2ShareLoading" and sufficient stock
      ...
      if (!productOrdersData || productOrdersData.length === 0) {
        console.log(
          "No product orders found with type 2ShareLoading and sufficient stock"
        );
        setAllVessels([]);
        return;
      }
      ...
      // Fetch vessels that have product orders with type "2ShareLoading" and sufficient stock
  ...
  useEffect(() => {
    if (productOrder.type === "2ShareLoading") {
  ...
  useEffect(() => {
    if (productOrder.type === "2ShareLoading" && buyerTrips.length > 0) {
